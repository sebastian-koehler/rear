...
if has_binary systemctl \
	...
	StopIfError $? "Unable to start vxpbx_exchanged via systemd"
	if test -e /etc/systemd/system/netbackup.service -o -e /usr/lib/systemd/system/netbackup.service ; then
		...
		StopIfError $? "Unable to start the NetBackup client via systemd"
	fi
elif test -x /etc/init.d/vxpbx_exchanged ; then
	/etc/init.d/vxpbx_exchanged start
	StopIfError $? "Unable to start vxpbx_exchanged via /etc/init.d/vxpbx_exchanged"
	if test -x /etc/init.d/netbackup ; then
		/etc/init.d/netbackup start
		StopIfError $? "Unable to start the NetBackup client via /etc/init.d/netbackup"
	fi
elif test -r /etc/xinetd.d/vnetd -o -r /etc/xinetd.d/bpcd -o -r /etc/xinetd.d/vopied ; then
	xinetd
	StopIfError $? "Unable to start xinetd"
...
nbu_primary=$( grep -i '^[[:space:]]*SERVER' /usr/openv/netbackup/bp.conf | head -1 | sed -e 's/^[^=]*=[[:space:]]*//' -e 's/[[:space:]]*$//' )
...
/usr/openv/netbackup/bin/nbcertcmd -getCAcertificate -server "$nbu_primary" 0<&6 1>&7 2>&8
StopIfError $? "Unable to fetch the NetBackup CA certificate from $nbu_primary"
...
while true ; do
	token=$( UserInput -I NBU_CERT_TOKEN -r -t 0 -p "Enter NetBackup enrollment token:" )
